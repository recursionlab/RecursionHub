...
"""
import os, sys, argparse, requests, collections, datetime
...
def create_backlog_issue(owner_repo, token, title, body):
    ...
    requests.post(url, headers=headers, json={"title": title, "body": body, "labels": ["knot"]})
...
def main():
    ...
    for title in frequent:
        ...
        create_backlog_issue(owner_repo, token, f"Knot: repeated PR title '{title}'", f"Detected {counts[title]} PRs with the same title in the last {args.days} days. Consider breaking cycles.")
