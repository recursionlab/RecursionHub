...
def compute(owner_repo, token):
    ...
    return {"holonomy_recent_merges": merged, "stalled_pr_count": stalled, "presence_score": score}
...
def main():
    ...
    if args.comment_if_low and args.pr_number and metrics["presence_score"] < 0.6 and token:
        # post a short comment on PR to warn/advise
        import requests
        headers = {"Authorization": f"token {token}"}
        body = {"body": f"Presence check: repository presence_score={metrics['presence_score']:.2f}. Consider adding context or tests."}
        requests.post(f"https://api.github.com/repos/{owner_repo}/issues/{args.pr_number}/comments", headers=headers, json=body)
