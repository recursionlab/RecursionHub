...
def get_pr(owner_repo, token, pr_number):
    ...
    resp = requests.get(
        f"{GITHUB_API}/repos/{owner_repo}/pulls/{pr_number}", headers=headers
    )
...
def post_comment(owner_repo, token, pr_number, body):
    headers = {"Authorization": f"token {token}"}
    requests.post(
        f"{GITHUB_API}/repos/{owner_repo}/issues/{pr_number}/comments",
        headers=headers,
        json={"body": body},
    )
...
def create_followup(owner_repo, token, title, body):
    ...
    requests.post(
        ...
        headers=headers,
        json={"title": title, "body": body, "labels": ["follow-up"]},
    )
...
def main():
    ...
    create_followup(
        owner_repo,
        token,
        f"Follow-up for sealed PR #{args.pr_number}",
        f"Auto-enqueued follow-up from sealed PR #{args.pr_number}\n\n{body}",
    )
